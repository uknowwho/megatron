...
def get_batch(data, context_length, batch_size):
    ...
    yb = torch.stack([data[sample+1:sample+context_length+1] for sample in samples])
...
def train(model, train_data, val_data, optimizer, batch_size=16, train_iters=5000, eval_iters=50):
    for iter in range(train_iters):
        model.train()
        batch_data, batch_targets = get_batch(train_data, model.context_length, batch_size)
        ...
        if iter % 100 == 0:
            train_loss, val_loss = estimate_loss(model, train_data, val_data, batch_size, eval_iters)
...
if __name__ == "__main__":
    ...
    model = Megatron(vocab_size, embed_length=32, context_length=64, dropout=0.15, num_heads=2, num_blocks=3)
    model.to(device)
    ...
    # print(decode(model.generate(torch.tensor(encode('Hallo')).view(1, 5), max_tokens=200).tolist()[0]))