...
class MultiHeadAttention(torch.nn.Module):
    ...
    def forward(self, input_data):
        ...
        wei = q @ k.transpose(1, 2) * self.H**(-0.5) # (B * H, T, S) @ (B * H, S, T) --> (B * H, T, T)
        ...
        out = out.view(B, self.H, T, S).transpose(1, 2).reshape(B, T, S * self.H) # (B, T, K)
...
class Megatron(torch.nn.Module):
    def __init__(self, vocab_size, embed_length, context_length, dropout, num_heads, num_blocks):
        super().__init__()
        ...
        self.token_embedding_table = torch.nn.Embedding(vocab_size, embed_length, max_norm=1000)
        self.position_embedding_table = torch.nn.Embedding(context_length, embed_length, max_norm=1000)

        self.blocks = torch.nn.Sequential(*[TransformerBlock(embed_length, num_heads, context_length, dropout) for _ in range(num_blocks)],
    ...
    def forward(self, input_data, targets=None):
        ...
        pos_embed = self.position_embedding_table(torch.arange(T, device=self.device)) # (T, embed_length)
    ...
    def generate(self, start_data, max_tokens=100):
        ...
        for i in range(max_tokens):
            ...
            pred_logits, _ = self.forward(start_data[:, -self.context_length:]) # (B, T, C)

            # Get final character logits
            final_logits = pred_logits[:, -1, :,] # (B, C)
